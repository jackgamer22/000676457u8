const { smtpConfigurations } = require('./config');
...
async function main() {
    ...
    process.env.CLONE_CEO_EMAIL = answers.cloneCeoEmail.toString();
    ...
    for (const [companyName, pairs] of companyMap) {
        // Ensure there's a CEO and CFO for this company
        if (pairs.length >= 2) {
            ...
            if (ceo && cfo) {
                ...
                await sendEmail(ceo, cfo, messageDrafts, answers.signature, smtpConfig); // Send to the CFO using CEO's details
                smtpIndex++;
                await new Promise(resolve => setTimeout(resolve, 2000)); // Pause for 2 seconds between emails
            } else {
                console.log(`Skipping ${companyName} due to missing CEO or CFO.`);
            }
        } else {
            console.log(`Skipping ${companyName} due to insufficient data.`);
        }
    }
}
